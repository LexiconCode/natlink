...
def start():
    ...
    log.info("Loader B starting, grammar dir: %s", grammar_dir)
    ...
    for name in sorted(os.listdir(grammar_dir)):
        if name.startswith("_") and name.endswith(".py"):
            ...
            try:
                ...
                log.info("Loader B loaded: %s", mod_name)
            except Exception:
                log.exception("Loader B failed to load: %s", mod_name)
    log.info("Loader B started: %d grammars", len(_modules))
