...
| 13b | **`natConnect` breaks `sys.stdout`/`sys.stderr.fileno()` process-wide** (measured). natlinkcore's `NewStdout`/`NewStderr.fileno()` raises `NotImplementedError` (`redirect_output.py:28`), and we install them on connect — **even when `discovered_loaders=[]`**. | same | Before connect `sys.stderr.fileno()` → `2`; after, it raises. Breaks `subprocess(..., stderr=sys.stderr)`, `logging` handlers wanting an fd, and any C-level consumer. The autouse `_ensure_connected` fixture fails this way, which accounts for the ~192 setup errors in a full live run (mechanism verified; per-test attribution inferred, not enumerated). Note `natlink_ui/_stream_redirect.py` is **implemented and unit-tested** (`tests/test_stream_redirect.py`) but wired into no production path — adopting it would let us implement `fileno()` properly. |
